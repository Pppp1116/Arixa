...
def main():
    try:
        ...
        return __astra_cast(len(bytes, data), 'i64')
...
def concat(a, b):
    try:
        result = __astra_cast(vec_new(), 'Vec<u8>')
        i = 0
        while (i < vec_len(a)):
            byte_opt = vec_get(a, i)
            if (byte_opt != None):
                vec_push(result, __astra_cast(byte_opt, 'u8'))
            i = (i + 1)
        i = 0
        while (i < vec_len(b)):
