...
from astra.optimizer_enhanced import optimize_program_enhanced, OptimizationContext
...
"""
    ...
    # Check if the invariant was computed correctly
    # Should have hoisted the invariant calculation
    cp = subprocess.run([sys.executable, str(out)], capture_output=True, text=True, timeout=5)
    
    # Look for constant folded result
    return_match = re.search(r"return (\d+)", code)
    if return_match:
        actual_computed = int(return_match.group(1))
        expected = 70  # 10 * 7
        assert actual_computed == expected, f"Expected {expected}, got {actual_computed}"
    else:
...
fn main() Int {
    ...
    c = x * 64;     // Should become x << 6
    d = x / 16;     // Should become x >> 4 (if unsigned)
    e = x % 8;      // Should become x & 7 (if unsigned)
    return a + b + c + d + e;
}
"""
    )
    ...
    # Check the actual computation in the generated code
    # The optimizations are working if we see the constant folded result
    expected = 5*16 + 5*32 + 5*64 + 5//16 + 5%8
    
    # Look for the return statement in the generated code
    return_match = re.search(r"return (\d+)", code)
    if return_match:
        actual_computed = int(return_match.group(1))
        assert actual_computed == expected, f"Expected {expected}, got {actual_computed}"
    else:
        # If we can't find the return statement, at least check it runs
        assert cp.returncode >= 0  # Should not crash
