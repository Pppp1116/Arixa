...
def test_register_ir_is_additive_across_calls():
    rt = AstraGpuRuntime()
    rt.register_ir({"kernels": [{"name": "a", "symbol": "sym_a", "params": [], "ret": "Void"}]})
    rt.register_ir({"kernels": [{"name": "b", "symbol": "sym_b", "params": [], "ret": "Void"}]})

    assert "sym_a" in rt._kernel_ir
    assert "a" in rt._kernel_ir
    assert "sym_b" in rt._kernel_ir
    assert "b" in rt._kernel_ir
