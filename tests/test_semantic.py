...
def test_vec_from_slice_infers_element_type():
    src = "fn main() Int{ mut v = vec_from([1, 2, 3]); vec_push(v, 4); return vec_len(v); }"
    analyze(parse(src))
...
def test_mutable_borrow_blocks_shared_borrow():
    ...
    try:
        ...
        assert "cannot create immutable reference to `x` because it is mutably borrowed" in str(e)
...
def test_shared_borrow_blocks_mutation():
    ...
    try:
        ...
        assert "cannot mutate `x` because it has 1 active immutable reference(s)" in str(e)
...
def test_mutable_borrow_blocks_direct_use_of_owner():
    ...
    try:
        ...
        assert "cannot read from `x` while it is mutably borrowed" in str(e)
...
def test_mutable_borrow_requires_mutable_binding():
    ...
    try:
        ...
        assert "cannot create mutable reference to `x` because it is declared as immutable" in str(e)
...
def test_move_while_shared_borrow_is_rejected():
    ...
    try:
        ...
        assert "cannot move `x` because it has 1 active immutable reference(s)" in str(e)
