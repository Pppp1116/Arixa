...
_astra_tid_lock = threading.Lock()
_astra_mutexes = {}
_astra_next_mutex = 1
_astra_channels = {}
_astra_next_chan = 1
...
def tcp_connect(addr):
    try:
        ...
        if addr_str.startswith('['):
            ...
            if end_bracket == -1:
                return -1
            host = addr_str[1:end_bracket]
            port_part = addr_str[end_bracket + 1:]
            if not port_part.startswith(':'):
                return -1
            port = int(port_part[1:])
        else:
            # IPv4 or hostname: 192.168.1.1:8080
...
class _AstraGpuNamespace:
    ...
    def _check_runtime(self):
        if self._runtime is None:
            raise RuntimeError("GPU runtime not initialized - _astra_gpu_runtime is None")
    def available(self):
        return False if self._runtime is None else self._runtime.available()
...
def main():
    ...
    try:
        i = 0
        while (i < 5):
            print_(astra_str_concat('Count: ', to_string_int(i)))
            i += 1
        return 0
...
def substring(s, start, length):
    ...
    try:
        ...
        if (((start_int < 0) or (length_int < 0)) or (start_int >= len(s_str))):
            return ''
        end = (start_int + length_int)
        if (end > len(s_str)):
            end = len(s_str)
        return s_str[start_int:end]
    except _AstraTryNone:
        return None
    except _AstraTryResultErr as __astra_err:
        return __astra_result_err(__astra_err.value)
...
def replace(s, pattern, replacement):
    _astra_defer_stack = []
    try:
        return str(s).replace(str(pattern), str(replacement))
    except _AstraTryNone:
        return None
    except _AstraTryResultErr as __astra_err:
        return __astra_result_err(__astra_err.value)
