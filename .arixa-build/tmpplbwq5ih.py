...
class AtomicInt:
    def __init__(self, handle):
        self.handle = handle
    def __repr__(self):
        return f"AtomicInt(handle={self.handle!r})"

def main():
    _astra_defer_stack = []
    try:
        a = atomic_int_new(1)
        prev = atomic_fetch_add(a, 5)
        return prev
...
def atomic_fetch_add(a, delta):
    ...
    try:
        ...
        for _d in reversed(_astra_defer_stack):
            _d()
def atomic_compare_exchange(a, expected, desired):
    _astra_defer_stack = []
    try:
