...
async def fetch_user(id):
    _astra_defer_stack = []
    try:
        await_result(sleep_ms(50))
        return astra_str_concat('User', to_string_int(id))
...
async def fetch_posts(user_id):
    _astra_defer_stack = []
    try:
        await_result(sleep_ms(30))
        return astra_str_concat('Posts for ', to_string_int(user_id))
    except _AstraTryNone:
        return None
    except _AstraTryResultErr as __astra_err:
        return __astra_result_err(__astra_err.value)
...
async def process_data(id):
    _astra_defer_stack = []
    try:
        user = await_result(fetch_user(id))
        posts = await_result(fetch_posts(id))
        return astra_str_concat(astra_str_concat(user, ': '), posts)
    except _AstraTryNone:
        return None
    except _AstraTryResultErr as __astra_err:
        return __astra_result_err(__astra_err.value)
...
def main():
    _astra_defer_stack = []
    try:
        result1 = await_result(process_data(1))
        print_(result1)
...
def sleep_ms(ms):
    ...
    try:
        ...
        for _d in reversed(_astra_defer_stack):
            _d()
def sleep_seconds(secs):
    _astra_defer_stack = []
    try:
        return sleep_ms((secs * 1000))
    except _AstraTryNone:
        return None
    except _AstraTryResultErr as __astra_err:
        return __astra_result_err(__astra_err.value)
