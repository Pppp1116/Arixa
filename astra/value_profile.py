...
def _collect_stmt_profiles(stmts: list[Any], fn_name: str, out: dict[str, dict[str, Any]]) -> None:
    for stmt in stmts:
        if isinstance(stmt, MatchStmt):
            ...
            _collect_stmt_profiles(getattr(stmt, 'body'), fn_name, out)
        for attr in ('expr', 'cond', 'init', 'step', 'target', 'right', 'left'):
            node = getattr(stmt, attr, None)
            if node is not None:
                _collect_expr_profiles(node, fn_name, out)
...
def _rewrite_stmts(stmts: list[Any], fn_name: str, profile: dict[str, dict[str, Any]]) -> list[Any]:
    ...
    for stmt in stmts:
        ...
        if isinstance(stmt, IfStmt):
            ...
            setattr(stmt, 'body', _rewrite_stmts(getattr(stmt, 'body'), fn_name, profile))
        out.append(stmt)
    return out
...
def _specialize_match_stmt(stmt: MatchStmt, fn_name: str, profile: dict[str, dict[str, Any]]) -> Any:
    ...
    setattr(fallback, '_value_specialized', True)
