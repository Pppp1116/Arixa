    ...
    for st in stmts:
        ...
        if isinstance(st, IfStmt):
            if _has_loop(st.then_body) or _has_loop(st.else_body):
                return True
        if hasattr(st, "body") and isinstance(getattr(st, "body"), list):
            if _has_loop(getattr(st, "body")):
                return True
    return False
...
def _set_variant_attrs(fn: ir.Function, suffix: str) -> None:
    ...
    if feat is None:
        return
    # llvmlite does not expose generic key/value attribute builders; keep a marker for diagnostics.
