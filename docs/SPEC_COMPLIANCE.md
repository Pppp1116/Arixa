...
| `fixed` vs `let mut` semantics | Checked in `semantic._analyze_block` and helpers (`_require_mutable_binding`, fixed scopes); `LetStmt.fixed` flag from parser | `tests/test_fixed_int_types.py::*` (assignment to `fixed`, fixed in `for` initializers), `tests/test_semantic.py::test_mutable_borrow_requires_mutable_binding` |
