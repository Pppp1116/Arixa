...
| Package manager (`astpm`) | partial | Add/remove/list/search/update/publish/lock/verify available with semver-aware transitive lock solving, source/checksum metadata, registry cache fallback for offline use, and richer per-constraint conflict diagnostics during version resolution. | Improve workspace-scale dependency ergonomics and keep conflict guidance actionable under large transitive graphs. | Lockfile includes deterministic transitive graph, and `astpm verify` validates cached module integrity. |
